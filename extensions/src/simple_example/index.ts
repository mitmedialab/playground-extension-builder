...
export default class SimpleTypescript extends extension(details, "ui", "customSaveData", "indicators") {
  ...
  @(scratch.command`This is what jibo looks like 2 ${{ type: "image", uri: jibo, alt: "Picture of Jibo", flipRTL: true }}`)
  imageBlock2(jibo: "inline image") {
  }

  @(scratch.reporter`${{ type: "number", defaultValue: 1 }} + PLUS + ${{ type: "image", uri: five, alt: "golden five" }} - ${"number"}`)
